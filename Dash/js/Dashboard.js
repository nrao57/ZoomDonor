...
function initDynamo(){ //NEED TO CHANGE WITH COGNITO
    AWS.config.update({
...
function scanItems(){
    ...
    var scanreq = docClient.scan(params, onScan);
    function onScan(err, data) {
        ...
        if (err) console.log(err);
        	...
        	var newdata = [];
        	data.Items.forEach(function(donor){
        		newdonor = {
        			projctid: donor.projctid,
        			date_posted: donor.info.date_posted,
        			grade_level: donor.info.grade_level,
        			resource_type: donor.info.resource_type,
        			funding_status: donor.info.funding_status,
        			poverty_level: donor.info.poverty_level,
        			school_state: donor.info.school_state,
        			total_donations: donor.info.total_donations
        		};
        		newdata.push(newdonor)
        	});
        	makeGraphs(newdata);
        }; 
        //data.Items is array of objects with key:values
        //data.Items[0] will give key:values for primary key and info object
